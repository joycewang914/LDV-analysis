...
var_pos3 = var_pos2 # save a var_pos copy with reference genome
...
Ali finished variant calling pipeline:
...
ind = which(unlist(lapply(vrefm_st, FUN = function(x){sum(x$tip.label %in% "5261-4089-0-RVRE") > 0})))
st_ind = unlist(vrefm_st_ldv[ind])

output[output$LDV.Position %in% names(which(apply(var_pos3[c("CP003351.1", "5261-4089-0-RVRE"), st_ind], 2, FUN = function(x){x[1] != x[2]}))), ]
```