...
```{r}
file = paste("../figures/", format(Sys.time(), "%Y-%m-%d"), '_R01_panel_figures_v2.pdf', sep = "");
pdf(file, width = 14)
par(mfrow = c(1,2))
...
nodelabels(pch = 21, col = "transparent", cex = 1, bg = nodecol)
...
plot(0, 0, xlab = "Spike-in relative abundance (%)", ylab = "Proportion", xaxt = "n", yaxt = "n", col = "white", xlim = c(1, 6), ylim = c(0, 1), main = "Proportion of nodes\nsupported by 50% LDVs", cex.main = 2, cex.lab = 1.5)
axis(side = 1, at = 1:6, labels = c("10", "5", "1", "0.1", "0.01", "0.001"), cex.axis = 1)
...
legend(x = "topright", legend = paste0("Strain ", 1:3), col = pch_col, pch = pch_shape, bty = "n", cex = 2)
