...
# bp = barplot(c(
#   mean(rel_ab_ldv[rel_ab_ldv$LDV.Position %in% unlist(ldv_i),"ALT.Allele.Frequency"])*100,
#   mean(rel_ab_ldv[!rel_ab_ldv$LDV.Position %in% unlist(ldv_i), "ALT.Allele.Frequency"])*100),
#   names.arg = c(paste0("LDVs (N = ", length(unlist(ldv_i)), ")"), 
...
cols = structure(colorRampPalette(c("white", "red"))(100), names = 1:100)
...
test_phylo_group_clade_end = sapply(test_phylo_group_clade, FUN = function(s){
  if (sum(phylo$edge[phylo$edge[,1] %in% s,2] < (length(phylo$tip.label)))){s_end = phylo$edge[phylo$edge[,1] %in% s,2][phylo$edge[phylo$edge[,1] %in% s,2] < (length(phylo$tip.label))][1]}else
    {s_end = ifelse(sum(phylo$edge[phylo$edge[,1] %in% s, 2] %in% test_phylo_group_clade), phylo$edge[phylo$edge[,1] %in% s, 2][which(phylo$edge[phylo$edge[,1] %in% s, 2] %in% test_phylo_group_clade)], s+1)
  }
})
  
edgecol_ind = unlist(apply(cbind(test_phylo_group_clade, test_phylo_group_clade_end), 1, FUN = function(x){
  which(phylo$edge[,1] == x[1] & phylo$edge[,2] == x[2])
}))

test_node_ind = unlist(test_node)
test_node_ind = test_node_ind[!is.na(test_node_ind) & test_node_ind > 0]
edgecol[edgecol_ind] = sapply(unlist(test_node_ind)*100, FUN = function(x){cols[x]})
...
pdf(file)
plot(phylo, use.edge.length = T,edge.color = edgecol, tip.color = tipcol, cex = 0.25)
nodelabels(col = "blue", cex = 0.25, frame = "none")
dev.off()
